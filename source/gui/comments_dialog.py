...
class CommentsDialog(wx.Dialog):
    def __init__(self, parent, comments):
        ...
        super().__init__(parent, title="Comments")

        self.CenterOnParent()
        self.SetSize(500, 500)
        p = wx.Panel(self)
        l1 = wx.StaticText(p, -1, "Comments")
        self.commentsBox = wx.ListBox(p, -1)
        closeButton = wx.Button(p, wx.ID_CLOSE, "Close")
    ...
    def displayComments(self):
        for comment in self.comments.comments['result'][self.count:]:
            comment = [comment['content'], "By {}".format(comment['author']['name'])]
    ...
    def loadMore(self):
        if self.comments.hasMoreComments and not self.isLoading:
            try:
                ...
                speak("Could not load more comments")
            finally:
                self.isLoading = False
        elif self.isLoading:
            speak("Still loading")
        else:
            speak("There are no more comments")
    ...
    def onComplete(self, event):
        self.displayComments()
        speak("More comments loaded")
    def contextSetup(self):
        contextMenu = wx.Menu()
        copyItem = contextMenu.Append(wx.ID_COPY, "Copy comment")
    ...
    def onCopy(self, event):
        ...
        if self.commentsBox.Selection != -1:
            ...
            speak("The selected comment was copied to the clipboard")
